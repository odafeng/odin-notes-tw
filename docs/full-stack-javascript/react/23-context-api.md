...
2. **有時 Context API 根本不是最佳解**：像元件組合（component composition）這種模式，往往能在不引入 context 的情況下解掉 prop drilling。另外，把 state 保持在離「真正使用它的元件」最近的位置（state colocation），不要過早、過度地往上提升，許多 prop drilling 從一開始就不會發生。
