...
最激進的一端。它把幾十個 HTML 元素的 `margin`、`padding`、`border` 通通歸零、`font-size` 統一成 `100%`，等於「焦土策略」。做完之後，`<p>` 不再看起來像段落、`<h1>` 不再看起來像標題、清單也沒有小圓點——所有元素在視覺上被拉平成同一種樣子。
...
所謂「opinionated（有主張／有偏好）」，指的就是這份 reset **做了超出「單純修不一致」之外的主觀選擇**：挑了特定字體、特定行高、加了工具性的規則。以 sanitize.css 為例，它就明確地把自己所做的每一個主觀決定一條一條寫進文件裡。整條光譜可以歸納成一句話：**從「全部移除」→「只做正規化」→「刻意的預設」**，在「一致性」和「實用性」之間找不同的平衡點。
...
!!! warning "reset 不是「越乾淨越好」——你會把無障礙也一起清掉"
    硬重置（如 Eric Meyer's Reset）把所有 `margin`、清單樣式歸零、`font-size` 統一成 `100%`，看似「乾淨」，但也把瀏覽器免費提供的可讀性與語意層次一起丟掉了。這麼做之後，`h1` 到 `h6` 看起來全都一樣大，你必須自己**重新**建立所有視覺層次。若你只是想抹平跨瀏覽器差異、又想保留合理預設，選 normalize 類的方案（或混合式）通常更省事。
