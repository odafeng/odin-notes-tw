...
兩者各有用武之地，除了參考上述差異，也要靠你自己判斷。舉例來說，如果你只是想在元素被啟用（active）時改變它的 opacity（不透明度），用 animation 就太over-kill（殺雞用牛刀）了；但如果你要處理更複雜的、有多個中間狀態或需要無限重複的效果，animation 才是正確的工具。
...
以下是一個可以直接開在瀏覽器裡執行的最小範例，示範 shorthand 寫法搭配中間關鍵影格。把它存成 `.html` 檔用瀏覽器開啟，就會看到一顆球在紅、藍、綠之間循環，並在中途放大。
...
!!! warning "animation 不需要觸發器，會立刻自動播放"
    與 transition 不同，animation 一旦定義好就會在頁面載入時立刻開始跑。如果你只是想在 `:hover` 時才觸發效果，直接把 animation 掛在元素上會導致它一進頁面就無限循環，這通常不是你要的結果。
