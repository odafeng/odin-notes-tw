...
### 為什麼需要 template engine（模板引擎）

為了讓 view 能動態呈現資料，我們會使用 **template engine（模板引擎）**。顧名思義，我們在程式碼裡寫的是 **template（模板）檔案**，這些檔案會在伺服器回應請求時被「轉換」成最終的 HTML。轉換的過程中，模板裡定義的變數會被替換成真正的資料。除此之外，我們還能在模板裡加入 **條件判斷（conditional）** 與 **迴圈（loop）** 邏輯——例如「使用者登入後才顯示他的名字」。這些是純 HTML 做不到的。當然，如果你還想要 client 端的互動，仍然可以照舊用 `<script>` 標籤加入客戶端 JavaScript（不過現階段我們不會在客戶端做太複雜的事）。

本課使用的 template engine 是 **EJS**（Embedded JavaScript templating）。EJS 的語法跟 HTML 非常接近，因此相較於其他模板引擎，學習曲線相對平緩。
...
接著在專案根目錄建立一個叫 `views` 的子資料夾，用來放模板檔案。
...
`app.set("view engine", "ejs")` 啟用 EJS 作為 view engine；`app.set("views", ...)` 告訴 app 到 `/views` 子資料夾去找模板。這裡用 `path.join(__dirname, "views")` 組出絕對路徑，`__dirname` 是目前這個檔案所在的資料夾，這樣不論你從哪裡啟動伺服器，路徑都會正確。

### EJS 語法：兩種標籤

EJS 最核心的就是兩組標籤：

- `<% ... %>`：讓你在模板裡 **執行 JavaScript**，例如寫條件判斷、`for` 迴圈、宣告變數。**它只執行、不輸出**。
...
實際跑一次。先在 `views` 子資料夾建立一個叫 `index.ejs` 的模板：
...
然後在 `app.js` 的某個 route（路由）裡把這個模板渲染出來：
...
如果打開瀏覽器的開發者工具檢視 HTML，會發現結構跟你寫的 EJS 模板一模一樣，只是 `message` 變數已經被換成它的值。

關鍵那一行是 `res.render("index", { message: "EJS rocks!" })`。因為我們先前已經設定好 `views` 與 `view engine` 這兩個屬性，`res.render` 的行為就被固定成：

- **第一個參數** `"index"`：到指定資料夾裡找一個叫 `index` 的模板（副檔名 `.ejs` 可以省略）。
- **第二個參數** `{ message: ... }`：一個物件，裡面的屬性會變成 **那個模板可以取用的變數**。

### EJS 裡的 locals 變數

上面那個例子，模板到底是「怎麼知道」有 `message` 這個變數的？當我們渲染 view 時，EJS 能取用兩個來源的資料：
...
這個 navbar 需要一個 `links` 變數。要把資料傳進去，就在渲染主模板時透過 `res.render()` 一起傳進 `locals`。修改 `app.js`，定義一個 `links` 陣列並傳給渲染 `index` 的 route handler：
...
跑一次伺服器就會看到 navbar 出現。這樣的可重用模板正是把 header、footer 放進每一頁的做法。

**注意這裡用的是 `<%-` 而不是 `<%=`**。`<%-` 是 **raw output（原始輸出）** 標籤，它會把內容原封不動地輸出、不做 HTML 跳脫（escape）。因為 `include` 產生的本身就是一段 HTML，如果用會跳脫的 `<%=`，那些 HTML 標籤會被「二次跳脫」變成純文字顯示出來，所以插入子模板時要用 `<%-`。
...
在 EJS 裡提供 static asset（靜態資源，例如 CSS、圖片、client 端 JS），跟之前直接寫 HTML 時很像：用 `<link>` 標籤把外部檔案掛進模板的 `<head>`。差別只在於——**app 需要知道去哪裡找這些資源**。在 `app.js` 設定：
...
// 渲染 index 模板，並把 links 傳進 locals
app.get("/", (req, res) => {
  res.render("index", { links: links });
});
...
!!! warning "直接寫變數名 vs. 寫 locals.變數名"
    如果你在模板裡取用一個 **沒有** 定義在 `res.render` 的 `locals` 引數、也不在 `res.locals` 裡的變數，兩種寫法的結果不同：
...
!!! warning "include 子模板要用 <%- 而不是 <%="
    `<%=` 會對輸出做 HTML escape（跳脫），把 `<`、`>` 等字元轉成 `&lt;`、`&gt;`。子模板 `include` 進來的本身就是 HTML，若用 `<%=` 會被「二次跳脫」，導致標籤變成純文字顯示。插入子模板一律用 raw output 標籤 `<%- include(...) %>`。反過來說，輸出「使用者提供的一般文字」時要用會跳脫的 `<%=`，才能避免 XSS（跨站腳本攻擊）。

!!! warning "靜態資源路徑不含 public"
    設定 `express.static(path.join(__dirname, "public"))` 後，`public` 就是資源的根目錄。在模板裡引用時要寫 `/styles.css`，**不是** `/public/styles.css`，多寫一層 `public` 反而會 404。
...
4. 建立一個可重用的 footer（頁尾）模板，並在 app 的所有 route 都渲染它。
