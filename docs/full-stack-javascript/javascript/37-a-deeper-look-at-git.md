...
到目前為止，你使用 Git 大概都停留在 `git add`、`git commit`、`git push` 這幾個指令的循環。它們就像是超強版的「存檔」按鈕，也讓多人協作變得順暢。Git 的指令其實不多，真正的困難往往在於「看不見 Git 在做什麼」。這一課的目標，就是幫你把 Git 底層發生的事「視覺化」，深入三個主題：改寫歷史（changing history）、指標（pointer）、以及在遠端（remote）情境下改寫歷史的風險。
