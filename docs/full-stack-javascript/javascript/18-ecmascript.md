...
新特性要進入規格，並不是委員會拍板就好，而是要經過一套公開透明的 **5 個階段（Stage 0 到 Stage 4）** 流程：從最初的點子（Stage 0），一路討論、寫規格、實作、測試，到最後定案（Stage 4）才正式納入年度版本。整個過程都公開在 TC39 的 GitHub 上，任何人都能追蹤提案進度。實務上，一個特性通常在 Stage 3 或 Stage 4 時，各家 JavaScript 引擎就會開始實作了。
