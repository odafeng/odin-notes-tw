...
要特別提醒：clone 時 GitHub 會提供 HTTPS 與 SSH 兩種網址，本課要用 **SSH**（形如 `git@github.com:USER-NAME/REPO.git`）。若你誤用 HTTPS（形如 `https://github.com/...`），之後 push 時會因為 GitHub 已在 2021 年停用密碼驗證而失敗。
...
1. `git push`（或更明確的 `git push origin main`）。若跳出密碼驗證的錯誤，代表你 clone 時用了 HTTPS，需切換 remote 為 SSH（見上方常見陷阱）。
